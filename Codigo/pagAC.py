...
def demo():
	...
	X_train, X_validation, Y_train, Y_validation = model_selection.train_test_split(X, Y, 
                                                                                ...
                                                                                random_state = 0,
                                                                                shuffle = True)
	Clasificacion = DecisionTreeClassifier(max_depth=8, min_samples_split=4, min_samples_leaf=2)
	...
	with st.spinner("Calculando árbol ..."):
		...
		st.pyplot(figura5)
	Reporte = export_text(Clasificacion, 
                      feature_names = ['Texture', 'Area', 'Smoothness', 
                                       'Compactness', 'Symmetry', 'FractalDimension'])
	...
	if(st.session_state.tecnico or st.session_state.normal):
		...
		if uploaded_file is not None:
			...
			if st.session_state.tecnico:
			  ...
			  if listo:
			    html = obtener_link("Árbol de Decisión - Clasificación",reglas,text,"ADC",True)
