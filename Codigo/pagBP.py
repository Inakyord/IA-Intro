...
reglas = Image.open("images/BP/bosque.png")
...
def iniciaBP():
	st.header("Bosque aleatorio - Pronóstico")
	st.subheader("Árboles de pronóstico")
	i,d = st.columns((1,2))
	with i:
		st.image(deco)
	with d:
...
def demo():
	...
	Estimador = PronosticoBA.estimators_[33]
	st.subheader("Árbol creado: ")
	with st.spinner("Calculando árbol ..."):
		figura5 = plt.figure(figsize=(16,16))  
		plot_tree(Estimador, feature_names = ['Texture', 'Perimeter', 'Smoothness', 
	                                         'Compactness', 'Symmetry', 'FractalDimension'])
		st.pyplot(figura5)
	Reporte = export_text(Estimador, feature_names = ['Texture', 'Perimeter', 'Smoothness', 
                                                     'Compactness', 'Symmetry', 'FractalDimension'])
...
def propioArchivo():
	...
	if(st.session_state.tecnico or st.session_state.normal):
		...
		if uploaded_file is not None:
			...
			PronosticoBA = RandomForestRegressor(n_estimators=estim, max_depth=prof, min_samples_split=inter, min_samples_leaf=hoja, random_state=0)
			PronosticoBA.fit(X_train, Y_train)
			#Se genera el pronóstico
			Y_Pronostico = PronosticoBA.predict(X_test)
			st.subheader("Modelo:")
			...
			Estimador = PronosticoBA.estimators_[n]
			st.subheader("Árbol creado: ")
			with st.spinner("Calculando árbol ..."):
				figura5 = plt.figure(figsize=(16,16))  
				plot_tree(Estimador, feature_names = columnas3)
				st.pyplot(figura5)
			Reporte = export_text(Estimador, feature_names = columnas3)
			with st.expander("Reporte de reglas:"):
				st.text(Reporte)
			reglas = (reglas + "\n\n\n"+"REPORTE: \n\n"+Reporte)
			...
			if st.session_state.tecnico:
			  ...
			  if listo:
			    html = obtener_link("Bosque aleatorio - Pronóstico",reglas,text,"BAP",True)
